...
**Task:** $\pi_{name} (lab3\_viewer \bowtie lab3\_watched \bowtie lab3\_movie)$
...
**Task:** $\pi_{viewer\_id} (\sigma_{score = 10 \text{ AND } genre = 'Horror'} watched \bowtie movie)$
...
**Task:** $\pi_{title} (\sigma_{age < 20} viewer \bowtie watched \bowtie movie)$
...
**任务:** $\pi_{title, duration} (\sigma_{genre = 'Action' \text{ AND } duration > 120} lab3\_movie)$
...
**任务:** $\pi_{name} (lab3\_viewer \bowtie lab3\_watched \bowtie lab3\_movie)$
*(列出所有看过电影的观众姓名)*
...
#### Q3: 复杂 RA 过滤
**任务:** $\pi_{viewer\_id} (\sigma_{score = 10 \text{ AND } genre = 'Horror'} watched \bowtie movie)$
...
#### Q20: 终极 RA 转 SQL 挑战
**任务:** $\pi_{title} (\sigma_{age < 20} viewer \bowtie watched \bowtie movie)$
*(找出 20 岁以下观众看过的电影标题)*
